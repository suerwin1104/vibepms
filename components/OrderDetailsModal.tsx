...
const OrderDetailsModal: React.FC<Props> = ({ room, hotel, reservation, onUpdateStatus, onIssueInvoice, onVoidInvoice, onPrint, onCollectPayment, onRefund, onTransferOverpayment, onCancelTransaction, transactions, billableItems, consumptionItems, invoices, currentUser, onCancelCheckout, onAddBillableItem, onDeleteBillableItem, onCheckout, initialCheckingOut }) => {
  ...
  const handleIssueInvoiceInternal = async () => {
    ...
    setIsIssuing(true);
    try {
      await onIssueInvoice(totalAmountDue, reservation.id, buyerId); // Pass buyerId
    } finally {
      setIsIssuing(false);
    }
  };
                  ...
                  {billableItems.map(item => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-[9px] text-slate-400 font-normal">數量: {item.quantity} | 備註: {item.note || '-'}</div>
                      </td>
                      <td className="px-6 py-4 text-right text-slate-800">
                        -${(item.amount * item.quantity).toLocaleString()}
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button onClick={() => onDeleteBillableItem(item.id)} className="text-rose-400 hover:text-rose-600">✕</button>
                      </td>
                    </tr>
                  ))}
                  {/* 顯示付款紀錄 (Credits) */}
            ...
            {reservation && (
              <div className="p-4 bg-slate-50 border-t border-slate-100">
                <div className="flex gap-2 items-end">
                  ...
                  <div className="w-20">
                    <input type="number" placeholder="$" value={newItem.amount} onChange={e => setNewItem({ ...newItem, amount: parseInt(e.target.value) || 0 })} className="w-full border border-slate-200 p-2 rounded-xl text-xs font-bold text-right" />
                  </div>
                  <div className="w-16">
                    <input type="number" min="1" value={newItem.quantity} onChange={e => setNewItem({ ...newItem, quantity: parseInt(e.target.value) || 1 })} className="w-full border border-slate-200 p-2 rounded-xl text-xs font-bold text-center" />
                  </div>
                  <div className="flex-1">
                    <input placeholder="備註" value={newItem.note} onChange={e => setNewItem({ ...newItem, note: e.target.value })} className="w-full border border-slate-200 p-2 rounded-xl text-xs font-bold" />
                  </div>
                  ...
                  }} className="bg-slate-800 text-white px-4 py-2 rounded-xl text-xs font-black shadow-lg hover:bg-black transition-all active:scale-95">
                    ＋
                  </button>
                </div>
              </div>
