...
import SortableHeader, { SortConfig, sortData } from './SortableHeader';
  ...
  return (
    <div className="space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">集團人員與授權管理</h1>
          <p className="text-slate-500 text-sm">管理全連鎖分店的櫃檯員帳號與存取權限。</p>
        </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingStaff ? "修改帳號授權" : "核發新員工授權"}>
        <StaffForm
          staff={editingStaff}
          hotels={hotels}
...
const StaffForm = ({ staff, hotels, isProcessing, onSubmit, departments, allStaff, systemRoles }: { staff: Staff | null, hotels: Hotel[], isProcessing: boolean, onSubmit: (data: any) => void, departments: Department[], allStaff: Staff[], systemRoles: SystemRole[] }) => {
  const [formData, setFormData] = useState({
    ...
    authorizedHotels: staff?.authorizedHotels || (hotels[0] ? [hotels[0].id] : []),
    email: staff?.email || '',
    lineId: staff?.lineId || '',
  ...
  const toggleHotel = (id: string) => {
    const next = formData.authorizedHotels.includes(id)
      ? formData.authorizedHotels.filter(h => h !== id)
      : [...formData.authorizedHotels, id];
    setFormData({ ...formData, authorizedHotels: next });
  };
  ...
  return (
    <form className="space-y-4" onSubmit={e => { e.preventDefault(); onSubmit(formData); }}>
      ...
      <div className="space-y-2">
        <label className="block text-xs font-bold text-slate-500 mb-1">授權分店 (多選)</label>
        <div className="grid grid-cols-2 gap-2">
          {hotels.map(h => (
            <button
              ...
              className={`p-3 rounded-xl border text-xs font-bold transition-all text-left flex items-center justify-between ${formData.authorizedHotels.includes(h.id) ? 'bg-blue-600 border-transparent text-white' : 'bg-slate-50 border-slate-100 text-slate-500'}`}
            >
              {h.name}
              {formData.authorizedHotels.includes(h.id) && <span>✓</span>}
            </button>
          ))}
        </div>
      </div>
