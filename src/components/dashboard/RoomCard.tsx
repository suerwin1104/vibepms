...
import { Room, RoomStatus, Housekeeper, Hotel, RoomTypeDefinition } from '../../types';
...
interface RoomCardProps {
  ...
  roomTypes?: RoomTypeDefinition[]; // 新增：房型定義
      ...
      {hasAssignment ? (
          ...
          {onAssign && onUnassign && (
            <div className="flex gap-1">
              ...
              >
                ...
                {activeHousekeepers.filter(h => h.name !== housekeeperName && (!h.authorizedBuildings?.length || (buildingId && h.authorizedBuildings.includes(buildingId)))).map(h => (
        ...
        onAssign && activeHousekeepers.length > 0 && (
          ...
          >
            ...
            {activeHousekeepers.filter(h => !h.authorizedBuildings?.length || (buildingId && h.authorizedBuildings.includes(buildingId))).map(h => (
              <option key={h.id} value={h.id}>{h.name}</option>
            ))}
          </select>
        )
...
const RoomCard: React.FC<RoomCardProps> = ({
  ...
  const typeName = roomTypes?.find(t => t.code === room.type)?.name || room.type;
    ...
    >
      ...
      <div className={`p-3 border-b flex justify-between items-center ${isReserved ? 'bg-blue-50/30' : 'bg-slate-50'}`}>
        <div className="flex flex-col items-start min-w-0 pr-2">
          <h3 className="text-xl font-black text-slate-800 group-hover:text-blue-600 transition-colors leading-none mb-0.5">{room.number}</h3>
          <span className="text-[10px] font-bold text-slate-400 leading-tight block break-words w-full">{typeName}</span>
        </div>
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex flex-col gap-1.5">
              <div className="font-black text-slate-900 text-sm leading-tight break-words">
                <span className="mr-1 inline-block">👤</span> {order.guestName}
              </div>
              <div className="flex items-center text-slate-400 bg-slate-50 px-2 py-1 rounded-lg w-fit border border-slate-100 mb-1">
                <SourceIcon source={order.source} />
                <span className="text-[10px] font-bold italic truncate max-w-[120px]">{order.source}</span>
              </div>
            </div>
                ...
                if (order.status === 'CheckedIn') {
                  ...
                  if (overdueHours > 0) {
                    ...
                    if (overdueHours >= thresholdHours) {
                      ...
                      lateFee = overdueHours * hourlyRate;
                    }
                  }
                }

                const balanceDue = (order.totalPrice || 0) + lateFee - (order.paidAmount || 0) - (order.discount || 0);
                return (
            ...
            {(() => {
              if (order.status !== 'CheckedIn') return null;
