...
interface Props {
    ...
    onRefreshDocumentTypes?: () => Promise<void>;
}
...
const WorkflowManagement: React.FC<Props> = ({
    ...
    onApprove, onReject, onAddWorkflow, onUpdateWorkflow, onDeleteWorkflow, onAddStep, onUpdateStep, onDeleteStep, onConvertPRtoPO, onRefreshDocumentTypes
}) => {
    // ... (existing state)
    const [subTab, setSubTab] = useState<'pending' | 'definitions' | 'forms' | 'history' | 'allDocs'>('pending');
    ...
    const [activeForms, setActiveForms] = useState<CustomForm[]>([]);
    const [applyingFormId, setApplyingFormId] = useState<string | null>(null);

    useEffect(() => {
        if (isAppModalOpen) {
            // Fetch active forms for the selector
            supabase.from('custom_forms').select('*').eq('is_active', true)
                .then(({ data }) => setActiveForms(data || []));
        }
    }, [isAppModalOpen]);
    ...
    useEffect(() => {
        const fetchDocumentDetails = async () => {
            ...
            switch (selectedInstance.documentType) {
                ...
                case 'CUSTOM_FORM': tableName = 'custom_form_submissions'; foreignKey = 'id'; break;
            }
    ...
    const renderDocumentItems = () => {
        const selectedInstance = workflowInstances.find(i => i.id === (selectedInstanceId || selectedDocId));

        if (loadingItems) return <div style={{ padding: '16px', textAlign: 'center', color: '#6b7280', fontSize: '13px' }}>載入中...</div>;
        if (documentItems.length === 0 && selectedInstance?.documentType !== 'CUSTOM_FORM') return (
            <div style={{ padding: '16px', textAlign: 'center', backgroundColor: '#f8fafc', borderRadius: '8px', color: '#9ca3af', fontSize: '13px' }}>
                無明細項目
            </div>
        );

        if (selectedInstance?.documentType === 'CUSTOM_FORM') {
            const submissionData = documentItems.length > 0 ? (documentItems[0].data || {}) : {};
            return (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', backgroundColor: '#f9fafb', padding: '16px', borderRadius: '8px' }}>
                    {Object.entries(submissionData).map(([key, value]) => (
                        <div key={key} style={{ padding: '8px', borderBottom: '1px solid #e5e7eb' }}>
                            <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '2px' }}>{key}</div>
                            <div style={{ fontSize: '14px', fontWeight: 500 }}>{String(value)}</div>
                        </div>
                    ))}
                    {Object.keys(submissionData).length === 0 && <div style={{ color: '#9ca3af', fontSize: '13px' }}>無表單資料</div>}
                </div>
            );
        }
        ...
        <div style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div style={{ display: 'flex', gap: '4px', backgroundColor: '#f1f5f9', padding: '4px', borderRadius: '10px', width: 'fit-content' }}>
                    {[{ key: 'pending', label: `📋 待簽核 (${pendingApprovals.length})` }, { key: 'definitions', label: '⚙️ 流程設定' }, { key: 'forms', label: '📝 表單設計' }, { key: 'history', label: '📜 我的簽核記錄' }, { key: 'allDocs', label: '📂 全部文件' }].map(tab => (
                ...
                <button
                    ...
                    style={{ padding: '10px 20px', backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 2px 4px rgba(59, 130, 246, 0.3)' }}
                >
                    ⚡ 發起申請
                </button>
            </div>
            ...
            {/* Custom Forms */}
            {subTab === 'forms' && <CustomFormManagement currentUser={currentUser} onRefresh={onRefreshDocumentTypes} />}

            {/* Application Modal */}
            {isAppModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
                    <div style={{ backgroundColor: '#f8fafc', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflowY: 'auto' }}>
                        {!applyingFormId ? (
                            <>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                    <h3 style={{ fontSize: '20px', fontWeight: 700 }}>發起新申請</h3>
                                    <button onClick={() => setIsAppModalOpen(false)} style={{ border: 'none', background: 'none', fontSize: '20px', cursor: 'pointer' }}>✕</button>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                                    {activeForms.map(form => (
                                        <div key={form.id} onClick={() => setApplyingFormId(form.id)}
                                            style={{ padding: '20px', backgroundColor: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}
                                            onMouseEnter={(e) => e.currentTarget.style.borderColor = '#3b82f6'}
                                            onMouseLeave={(e) => e.currentTarget.style.borderColor = '#e2e8f0'}
                                        >
                                            <div style={{ fontWeight: 600, fontSize: '16px', marginBottom: '4px' }}>{form.title}</div>
                                            <div style={{ color: '#6b7280', fontSize: '13px' }}>{form.description || '無說明'}</div>
                                        </div>
                                    ))}
                                    {activeForms.length === 0 && <div style={{ colSpan: 2, textAlign: 'center', padding: '20px', color: '#9ca3af' }}>目前沒有可用的表單</div>}
                                </div>
                            </>
                        ) : (
                            <CustomFormRenderer
                                formId={applyingFormId}
                                currentUser={currentUser}
                                onSuccess={() => { setIsAppModalOpen(false); setApplyingFormId(null); setSubTab('allDocs'); }}
                                onCancel={() => setApplyingFormId(null)}
                            />
                        )}
                    </div>
                </div>
                        ...
                        {!applyingFormId ? (
                            <>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                                    ...
                                    {activeForms.length === 0 && <div style={{ gridColumn: 'span 2', textAlign: 'center', padding: '20px', color: '#9ca3af' }}>目前沒有可用的表單</div>}
                                </div>
                            </>
                        ) : (
                            <CustomFormRenderer
