  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (txData.length > 0) {
        ...
        if (txError) {
          addLog(`⚠️ 交易插入失敗: ${txError.message}`);
          console.error('Transaction insert error:', txError);
        } else {
          addLog(`✅ 已插入 ${txData.length} 筆交易`);
        }
      }
      if (invData.length > 0) {
        addLog(`📋 準備插入 ${invData.length} 筆發票...`);
        console.log('Invoice data to insert:', invData);
        const { error: invError } = await supabase.from('invoices').insert(invData);
        if (invError) {
          addLog(`❌ 發票插入失敗: ${invError.message}`);
          console.error('Invoice insert error:', invError);
        } else {
          addLog(`✅ 已成功插入 ${invData.length} 筆發票`);
        }
      } else {
        addLog(`⚠️ 沒有需要插入的發票資料 (CheckedOut 預約數量: ${resData.filter((r: any) => r.status === 'CheckedOut').length})`);
      }
