...
const HousekeepingOps: React.FC<HousekeepingOpsProps> = ({
  ...
  const [subTab, setSubTab] = useState<'DailyStatus' | 'Assignment' | 'Performance' | 'History' | 'MonthlyStats' | 'DailyTrack'>('DailyStatus');
  const [filterStatus, setFilterStatus] = useState<string>('all');

  const today = new Date().toISOString().split('T')[0];
  const todayTasks = cleaningTasks.filter(t => t.taskDate === today);

  // Deduplicate tasks: prevents identical visual clones (same room, status, type, and person)
  // while allowing legitimate sequential tasks for the same room.
  const uniqueTodayTasks = useMemo(() => {
    const seen = new Set<string>();
    return todayTasks.filter(task => {
      // Logic Duplication Key: same room + same status + same type + same assigned staff
      const signature = `${task.roomId}-${task.status}-${task.taskType}-${task.housekeeperId || 'none'}`;
  ...
  const summaryStats = useMemo(() => ({
    total: uniqueTodayTasks.length,
    unassigned: uniqueTodayTasks.filter(t => t.status === 'Unassigned').length,
    inProgress: uniqueTodayTasks.filter(t => t.status === 'InProgress').length,
    completed: uniqueTodayTasks.filter(t => t.status === 'Completed' || t.status === 'Inspected').length,
  }), [uniqueTodayTasks]);
  ...
  const filteredTasks = (filterStatus === 'all'
    ? uniqueTodayTasks
    : uniqueTodayTasks.filter(t => t.status === filterStatus)
              ...
              <div className="space-y-2">
                ...
                {task.housekeeperName && task.status !== 'Unassigned' ? (
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-1">
                      ...
                      >
                        ...
                        {housekeepers.filter(h => h.status === 'Active' && h.name !== task.housekeeperName && (!h.authorizedBuildings?.length || (room?.buildingId && h.authorizedBuildings.includes(room.buildingId)))).map(h => (
                  ...
                  >
                    ...
                    {housekeepers.filter(h => h.status === 'Active' && (!h.authorizedBuildings?.length || (room?.buildingId && h.authorizedBuildings.includes(room.buildingId)))).map(h => (
                      <option key={h.id} value={h.id}>{h.name}</option>
                    ))}
                  </select>
                )}
              ...
              <div key={task.id} className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm hover:border-rose-300 transition-colors flex justify-between items-center group">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    ...
                    {housekeepers.filter(h => {
                      const taskRoom = rooms.find(r => r.id === task.roomId);
                      return h.status === 'Active' && (!h.authorizedBuildings?.length || (taskRoom?.buildingId && h.authorizedBuildings.includes(taskRoom.buildingId)));
